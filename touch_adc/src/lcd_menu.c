...
void lcd_menu(void)
{
    ...
    while (1) {
        ...
        printf("[l] Draw line\n\r");
        printf("[f] Show the font test\n\r");
        printf("[a] Get adc value\n\r");
        printf("[p] Touch down or up test\n\r");
        printf("[q] quit\n\r");
...
void touch_screen_test(void)
{
    ...
    /* 显示校准屏幕的字体 */
    font_init();
    print_font_string(70, 70, "Touch screen calibrate", 0xff0000);
    ...
    while(1) {
        /* 通过触摸屏的值转换为LCD屏幕的值 */
        if (ts_to_lcd_read(&lcd_x, &lcd_y, &press_st) == 0) {
            printf("x = %d, y = %d\n\r", lcd_x, lcd_y);
            if (press_st) {
                /* 画点 */
                paint_pot(lcd_x, lcd_y, 0xff00);
            }
        }   
    }
}
